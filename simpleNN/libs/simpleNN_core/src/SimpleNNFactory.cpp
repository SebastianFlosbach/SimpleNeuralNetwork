...
ISimpleNN_ptr SimpleNNFactory::createSimpleNN(const SimpleNNData& data) {
	...
	for (size_t i = 0; i < data.size() - 1; i++) {
		Eigen::MatrixXf connectionMatrix = Eigen::MatrixXf::Identity(layerData[i + 1], layerData[i]);
		Eigen::VectorXf biasVector = Eigen::VectorXf::Random(layerData[i]);
		layers[i] = Layer( connectionMatrix, biasVector );
	}

	Eigen::MatrixXf connectionMatrix = Eigen::MatrixXf::Random(layerData[data.size() - 1], layerData[data.size() - 1]);
